    ...
    components: {
     ...
     methods:{
       ...
       transTreeData(datas,arr){
         if(datas.length){
            for(var i=0;i<datas.length;i++){
              ...
              if(data.flag == "XQ"){
                customType = 0
              }else if(data.flag == "部门" || data.flag.startsWith('N')){
                customType = 1
              }else if(data.flag.startsWith('B')){
                customType = 2
              }
               arr.push({"id":data.treeCid,"parentId":parentId,"name":data.label,"customType":customType,open: false,flag:data.flag,valueStr:data.valueStr})
              if(data.children){
      ...
      onCheck(evt, treeId, treeNode){
        var checkedNodes = this.$refs.accountZtreeRef.ztreeObj.getCheckedNodes();
        let treeData = [];
