  ...
  export default{
    ...
    components: {
     ...
     methods:{
       ...
       getGradeclassZtreeInit(){
         getGradeclassZtreeData().then(res => {
           var arr = [];
       ...
       transTreeData(datas,arr){
         if(datas.length){
            for(var i=0;i<datas.length;i++){
              ...
              var customType = ((data.flag == "XQ" || data.flag == 'SCHOOL') ? 0 : data.flag.startsWith("N") ? 1 : 2)
       ...
       onCheck(evt, treeId, treeNode){
        debounce(() => {
          var checkedNodes = this.$refs.gradclassZtreeRef.ztreeObj.getCheckedNodes();
          let treeData = [];
