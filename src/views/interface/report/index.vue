...
import reportAddEdit from './report-add-edit.vue';
...
export default {
  ...
  methods: {
    handleImport(){
      this.$refs.importInfoRef.openDialog('reportSql')
    },
    handleExport(){
      if(this.selectionList.length === 0) return this.$message.warning('请选择至少一条数据');
      this.$confirm('是否导出报表接口?', '提示', {
    ...
    handleAdd() {
      this.$refs.reportAddEditRef.openDialog({ sqlType: 1 });
    },
    handleEdit() {
      ...
      cussqlDetails({id:this.selectionList[0].id}).then(r => {
        if(r.data.code === 200){
          ...
          this.$refs.reportAddEditRef.openDialog({sqlType:1,details:details});
        }
      })
    },
    handleDelete() {
