<template>
  <el-drawer
    ...
    <avue-form ref="formRef" :option="option" v-model="form" @submit="submit" @error="error">
      ...
      <template slot-scope="scope" slot="LEAVETIME_MIN_MINUTE">
        <div>
          <el-input-number v-model="form.LEAVETIME_MIN_MINUTE" :min="0" :max="9999" :precision="1" :controls="false" class="number_input"></el-input-number>
          <span class="apendClass" style="height: 30px;line-height: 30px;">小时</span>
          -
          <el-input-number v-model="form.LEAVETIME_MAX_MINUTE" :min="0" :max="9999" :precision="1" :controls="false" class="number_input"></el-input-number>
          <span class="apendClass" style="height: 30px;line-height: 30px;">小时</span>
        </div>
      </template>
    </avue-form>
  ...
  methods: {
    ...
    submit(form, done) {
      this.$refs.formRef.validate((vaild, done) => {
        if (vaild) {
          if(form.LEAVETIME_MAX_MINUTE < form.LEAVETIME_MIN_MINUTE){
            this.$message.warning('请假时长范围填写不正确')
            done();
            return;
          }
...
.number_input{width: calc(50% - 66px);}
