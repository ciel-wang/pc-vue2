...
import { getClassesDeptList, classesDeptUpdata } from '@/api/kq/classesgroup.js';
...
export default {
  ...
  methods: {
    ...
    submit(form, done) {
      this.$refs.formRef.validate((vaild, done) => {
        if (vaild) {
          this.editForm['deptIds'] = this.ids;
          classesDeptUpdata(this.editForm).then(r => {
            if (r.data.code == 200) {
    ...
    onLoad() {
      ...
      this.loading = true;
      getClassesDeptList(params).then(r => {
        this.loading = false;
        if (r.data.code == 200) {
          let d = r.data.data;
    ...
    handleExport() {
      this.$confirm('是否导出部门分组?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        window.open(
          `/api/iot-kq/classes/dept/export?${this.website.tokenHeader}=${getToken()}&ids=${this.ids}&moreParamQuery=${this.moreParamQuery.trim()}&size=${this.page.pageSize}&current=${this.page.currentPage}`
        );
      });
    },
    escForm() {
  ...
  computed: {
    ...
    permissionList() {
      return {
        editBtn: this.vaildData(this.permission.kq_classesgroup_dept_edit_btn, false),
        exportBtn: this.vaildData(this.permission.kq_classesgroup_dept_export_btn, false)
      };
    },
    ids() {
      let ids = [];
