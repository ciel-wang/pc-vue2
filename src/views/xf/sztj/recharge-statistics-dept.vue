  ...
  <div>
    <div style="margin-bottom: 8px;">
      ...
      <el-button
        type="primary"
        size="small"
        v-if="permission.sztj_dept_export_btn"
...
import { getXfStatistticsDeptList } from '@/api/xf/szdetails';
...
export default {
  ...
  methods: {
    onLoad(page, params = {}) {
      ...
      getXfStatistticsDeptList(this.page.currentPage, this.page.pageSize, params).then(res => {
    ...
    handleExport() {
      this.$confirm('是否导出当前收支统计信息?', '提示', {
        ...
        window.open(
          `/api/iot-xf/szstatistics/dept/list/export?current=${this.page.currentPage}&size=${this.page.pageSize}&showType=${showType}&accTransStartTime=${accTransStartTime}&accTransEndTime=${accTransEndTime}&${this.website.tokenHeader}=${getToken()}&moreParamQuery=${this.moreQueryParam}`
        );
      });
    },
    sumNum(list, field) {
