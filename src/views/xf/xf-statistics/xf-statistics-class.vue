<template>
  <div>
    <div style="margin-bottom: 8px;">
      <el-select v-model="walletNum" clearable size="small" placeholder="选择钱包" style="width: 160px;">
      ...
      <el-button type="primary" size="small" icon="" @click="handleExport" v-if="permission.statistics_class_export_btn">导出</el-button>
    </div>
    <vxe-grid
      v-show="activeName == 'class'"
...
import { getXfStatisticsByClass } from '@/api/xf/xfdetails';
...
export default {
  ...
  methods: {
    ...
    onLoad(params = {}) {
      ...
      getXfStatisticsByClass(params).then(res => {
    ...
    statisticsData() {
      ...
      // updateDeptXfStatistics(params).then(res => {
      //   this.$message({
      //     type: 'success',
      //     message: '数据统计成功'
      //   });
    ...
    handleExport() {
      this.$confirm('是否导出当前消费统计信息?', '提示', {
        ...
        window.open(
          `/api/iot-xf/xfstatistics/class/list/export?current=${this.page.currentPage}&size=${this.page.pageSize}&walletNum=${this.walletNum}&showType=${showType}&accTransStartTime=${accTransStartTime}&accTransEndTime=${accTransEndTime}&${this.website.tokenHeader}=${getToken()}&ids=${ids}&moreParamQuery=${this.moreQueryParam.trim()}`
        );
      });
    }
  }
