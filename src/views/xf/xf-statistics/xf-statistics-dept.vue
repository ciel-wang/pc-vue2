<template>
  <div>
    <div style="margin-bottom: 8px;">
      ...
      <el-button type="primary" size="small" icon="" @click="handleExport" v-if="permission.statistics_dept_export_btn">导出</el-button>
    </div>
    <vxe-grid
...
import { getXfStatisticsByDept, updateDeptXfStatistics } from '@/api/xf/xfdetails';
...
export default {
  ...
  methods: {
    ...
    onLoad(page, params = {}) {
      ...
      getXfStatisticsByDept(this.page.currentPage, this.page.pageSize, params).then(res => {
    ...
    handleExport() {
      this.$confirm('是否导出当前消费统计信息?', '提示', {
        ...
        window.open(
          `/api/iot-xf/xfstatistics/dept/list/export?current=${this.page.currentPage}&size=${this.page.pageSize}&walletNum=${this.walletNum}&showType=${showType}&accTransStartTime=${accTransStartTime}&accTransEndTime=${accTransEndTime}&${this.website.tokenHeader}=${getToken()}&ids=${ids}&moreParamQuery=${this.moreQueryParam.trim()}`
        );
      });
    }
  }
