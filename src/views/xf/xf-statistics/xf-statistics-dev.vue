<template>
  ...
  <div>
    <div style="margin-bottom: 8px;">
      ...
      <el-button type="primary" size="small" icon="" @click="handleExport" v-if="permission.statistics_dev_export_btn">导出</el-button>
    </div>
    <vxe-grid
      v-show="activeName == 'dev'"
...
import { getXfStatisticsByDev, updateDevXfStatistics } from '@/api/xf/xfdetails';
...
export default {
  ...
  methods: {
    ...
    statisticsData() {
      ...
      updateDevXfStatistics(params).then(res => {
        this.$message({
          type: 'success',
          message: '数据统计成功'
        });
    ...
    onLoad(page, params = {}) {
      ...
      getXfStatisticsByDev(this.page.currentPage, this.page.pageSize, params).then(res => {
    ...
    handleExport() {
      this.$confirm('是否导出当前消费统计信息?', '提示', {
        ...
        window.open(
          `/api/iot-xf/xfstatistics/dev/list/export?current=${this.page.currentPage}&size=${this.page.pageSize}&walletNum=${this.walletNum}&showType=${showType}&accTransStartTime=${accTransStartTime}&accTransEndTime=${accTransEndTime}&${this.website.tokenHeader}=${getToken()}&ids=${ids}&moreParamQuery=${this.moreQueryParam.trim()}`
        );
      });
    }
  }
