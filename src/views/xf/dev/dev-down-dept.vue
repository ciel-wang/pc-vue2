      ...
      <el-row>
        <el-col :span="7">
          <div style="height:682px;overflow:auto;padding: 15px 0 15px 15px;border: 1px solid #eee;margin: 8px;">
            <dept-ztree ref="deptZtreeRef" isCheck @onCheck="onCheckDept" style="width: 100%;height: 100%;overflow: auto;"></dept-ztree>
          </div>
        </el-col>
...
import { getEventDeptList, faceSellerDeptPersonDown } from '@/api/xf/event';
export default {
  name: 'dev-dpwn-dept',
  ...
  methods: {
    handleSubmit() {
      ...
      let detpNos = this.currRows.map(v => v.deptNo).join(',');
      ...
      if (detpNos == '') return this.$message.warning('请选择下载部门');
      var param = {
        devIds: this.devIds,
        detpNos: detpNos,
        downCon: downCon,
      ...
      faceSellerDeptPersonDown(param).then(res => {
    ...
    onLoad(params = {}) {
      params['ancestors'] = this.ids;
      params['isFace'] = this.isFace;
      params['isCard'] = this.isCard;
      params['moreQueryParam'] = this.moreQueryParam
      params['filterType'] = this.filterType;
      this.empLoading = true;
      getEventDeptList(this.page.currentPage, this.page.pageSize, params).then(res => {
