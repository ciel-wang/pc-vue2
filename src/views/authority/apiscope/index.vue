<template>
  <basic-container>
    ...
    <el-drawer :title="`[${scopeMenuName}] 接口权限配置`" :visible.sync="drawerVisible" :direction="direction" append-to-body :before-close="handleDrawerClose" size="1000px">
      <basic-container>
        <avue-crud
          :option="optionScope"
          :data="dataScope"
          ...
          <template slot="menuLeft">
            <el-button type="danger" size="small" icon="el-icon-delete" plain @click="handleDeleteScope">删 除</el-button>
          </template>
...
export default {
  ...
  methods: {
    ...
    rowSaveScope(row, done, loading) {
      row = { ...row, menuId: this.scopeMenuId };
    ...
    onLoadScope(page, params = {}) {
      ...
      const values = { ...params, menuId: this.scopeMenuId };
